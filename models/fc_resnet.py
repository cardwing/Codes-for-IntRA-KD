...
__all__ = ['FCResNet', 'fc_resnet18', 'fc_resnet34', 'fc_resnet50', 'fc_resnet101', 'fc_resnet152']
...
class FCResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        #self.conv_2 = nn.Conv2d(512, 2048, kernel_size=1, stride=1, padding=0, bias=False) # newly added
        #self.conv_3 = nn.Conv2d(1024, 2048, kernel_size=1, stride=1, padding=0, bias=False) # newly added
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        #x4 = x
        #x = (x4 + self.conv_3(x3) + self.conv_2(x2)) / 3.0
    ...
    def load_state_dict(self, state_dict):
        state_dict.popitem()
        state_dict.popitem()
        super(FCResNet, self).load_state_dict(state_dict)


def fc_resnet18(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FCResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def fc_resnet34(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FCResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


def fc_resnet50(pretrained=True, **kwargs):
...
def fc_resnet152(pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = FCResNet(Bottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
